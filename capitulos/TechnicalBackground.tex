...
Figure \ref{fig:autonomous-vehicles} shows the main vehicles' sensors, and in the next subsection each sensor is described. 
...
Equation \ref{eq:eq_unit} shows the behavior of the Unit Step activation function ($H(x)$) where it is possible to note that it is nonlinear, monotonic, and fine for classification, but in other hands it has discontinuous derivatives and not monotonic with gradient descent methods.
...
Equation \ref{eq:sign} shows the behavior of the Sign Function activation function. Its properties are similar to the (\ref{eq:eq_unit}) as they have, essentially, the same format with different values. The pros are nonlinear, monotonic, and ideal for fine classification, but the cons are discontinuous derivative and it is not suitable for regressions.
