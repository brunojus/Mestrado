...
When it is necessary to work with variables that contain many contents, there is a necessity to improve this work and reduce computer bottleneck during the process. In machine learning (ML), some variables are independents or some cases features on which the final output is done. Moreover, in other cases, that number of these features increases it and reduces the ability to visualize it. 
...
To brief these steps is necessary to assume the more variability in a particular direction correlates with explaining the behavior of the dependent variable. Theoretically, it is necessary to apply the PCA to remove the noise in the sample and keep only the essential things that are necessary to detect.
...
The confidence loss is used to measure objectness of the box because a grand part of the boxes does not have any detector inside, and with this, an imbalance issue is noted, to avoid this object is necessary to compute this loss. 
...
\end{figure}
 ...
 The creation of a scenario-based in multiple cameras, a formation of an array of cameras, and a command center are responsible for merging all of the collected data and fusing this data on the database, such as the label of each object, position, and its timestamp. 
...
Inverse perspective mapping is a mathematical technique that removes the effects of a picture's distortion when transforming the perspective of the image to another perspective. Despite disparity mapping, the inverse perspective mapping method requires only one camera, and this method cannot provide depth information directly ~\cite{Tuohy2010}.
...
Figure \ref{fig:networkBehavior} shows how the model performs the inference process along the detection time.  
